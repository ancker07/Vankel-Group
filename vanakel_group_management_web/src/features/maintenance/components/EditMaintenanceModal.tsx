        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">INTERVENTION DESCRIPTION (OPTIONAL)</label>
              <button
                type="button"
                onClick={handleImprove}
                disabled={isImproving || !description}
                className="flex items-center gap-1.5 text-orange-500 text-[9px] font-black uppercase tracking-widest hover:bg-orange-500/10 px-2 py-1 rounded transition-all disabled:opacity-50 min-h-[30px]"
              >
                {isImproving ? <Loader2 size={12} className="animate-spin" /> : <Sparkles size={12} />} {isImproving ? (t.working || 'Working...') : (t.improveAI_text || 'Improve AI')}
              </button>
            </div>
            <textarea
              value={description}
